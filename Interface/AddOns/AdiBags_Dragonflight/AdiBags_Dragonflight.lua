...
by Zottelchen
version: 1.1.15
Add various Dragonflight items to AdiBags filter groups
]]
...
local GeneralCraftingReagentsIDs = {
...
local DrakewatcherManuscriptsIDs = {
...
191472, -- Writhebark
 ...
 -- Elemental Trade Goods - Rousing
local ElementalTradeGoodsRousingIDs = {
...
202016, -- Saturated Bone
 ...
 -- Elemental Trade Goods - Awakened
local ElementalTradeGoodsAwakenedIDs = {
190316, -- Awakened Earth
190321, -- Awakened Fire
190324, -- Awakened Order
190327, -- Awakened Air
...
local function MatchIDs_Init(self)
    ...
    if self.db.profile.moveInscription then
        if self.db.profile.showcoloredCategories then
            Result["|cff4dfaf8Inscription|r"] = AddToSet(InscriptionIDs)
        else
            Result[unescape("|cff4dfaf8Inscription|r")] = AddToSet(InscriptionIDs)
        end
    end

    if self.db.profile.movePotions then
        if self.db.profile.showcoloredCategories then
            Result["|cffebb134Potions|r"] = AddToSet(PotionsIDs)
        else
            Result[unescape("|cffebb134Potions|r")] = AddToSet(PotionsIDs)
        end
    end

    if self.db.profile.moveCooking then
        if self.db.profile.showcoloredCategories then
            Result["|cffff0072Cooking|r"] = AddToSet(CookingIDs)
        else
            Result[unescape("|cffff0072Cooking|r")] = AddToSet(CookingIDs)
        end
    end

    if self.db.profile.moveGeneralCraftingReagents then
        if self.db.profile.showcoloredCategories then
            Result["|cff51479cGeneral Crafting Reagents|r"] = AddToSet(GeneralCraftingReagentsIDs)
        else
            Result[unescape("|cff51479cGeneral Crafting Reagents|r")] = AddToSet(GeneralCraftingReagentsIDs)
        end
    end

    if self.db.profile.moveDrakewatcherManuscripts then
        if self.db.profile.showcoloredCategories then
            Result["|cff33937fDrakewatcher Manuscripts|r"] = AddToSet(DrakewatcherManuscriptsIDs)
        else
            Result[unescape("|cff33937fDrakewatcher Manuscripts|r")] = AddToSet(DrakewatcherManuscriptsIDs)
        end
    end

    if self.db.profile.moveDarkmoonCards then
        if self.db.profile.showcoloredCategories then
            Result["|cff48aa00Darkmoon Cards|r"] = AddToSet(DarkmoonCardsIDs)
        else
            Result[unescape("|cff48aa00Darkmoon Cards|r")] = AddToSet(DarkmoonCardsIDs)
        end
    end

    if self.db.profile.movePreEventCurrency then
        if self.db.profile.showcoloredCategories then
            Result["|cff0060a2PreEvent Currency|r"] = AddToSet(PreEventCurrencyIDs)
        else
            Result[unescape("|cff0060a2PreEvent Currency|r")] = AddToSet(PreEventCurrencyIDs)
        end
    end

    if self.db.profile.moveHerbs then
        if self.db.profile.showcoloredCategories then
            Result["|cff00ff36Herbs|r"] = AddToSet(HerbsIDs)
        else
            Result[unescape("|cff00ff36Herbs|r")] = AddToSet(HerbsIDs)
        end
    end

    if self.db.profile.moveElementalTradeGoodsRousing then
        if self.db.profile.showcoloredCategories then
            Result["|cff644bffElemental Trade Goods - Rousing|r"] = AddToSet(ElementalTradeGoodsRousingIDs)
        else
            Result[unescape("|cff644bffElemental Trade Goods - Rousing|r")] = AddToSet(ElementalTradeGoodsRousingIDs)
        end
    end

    if self.db.profile.moveElementalTradeGoods then
        if self.db.profile.showcoloredCategories then
            Result["|cff644bffElemental Trade Goods|r"] = AddToSet(ElementalTradeGoodsIDs)
        else
            Result[unescape("|cff644bffElemental Trade Goods|r")] = AddToSet(ElementalTradeGoodsIDs)
        end
    end

    if self.db.profile.moveLeather then
        if self.db.profile.showcoloredCategories then
            Result["|cff865b00Leather|r"] = AddToSet(LeatherIDs)
        else
            Result[unescape("|cff865b00Leather|r")] = AddToSet(LeatherIDs)
        end
    end

    if self.db.profile.moveJewelcrafting then
        if self.db.profile.showcoloredCategories then
            Result["|cff514dfaJewelcrafting|r"] = AddToSet(JewelcraftingIDs)
        else
            Result[unescape("|cff514dfaJewelcrafting|r")] = AddToSet(JewelcraftingIDs)
        end
    end

    if self.db.profile.moveCloth then
        if self.db.profile.showcoloredCategories then
            Result["|cffff7e00Cloth|r"] = AddToSet(ClothIDs)
        else
            Result[unescape("|cffff7e00Cloth|r")] = AddToSet(ClothIDs)
        end
    end

    if self.db.profile.moveMining then
        if self.db.profile.showcoloredCategories then
            Result["|cff898396Mining|r"] = AddToSet(MiningIDs)
        else
            Result[unescape("|cff898396Mining|r")] = AddToSet(MiningIDs)
        end
    end

    if self.db.profile.moveProfessionKnowledge then
        if self.db.profile.showcoloredCategories then
            Result["|cff33937fProfession Knowledge|r"] = AddToSet(ProfessionKnowledgeIDs)
        else
            Result[unescape("|cff33937fProfession Knowledge|r")] = AddToSet(ProfessionKnowledgeIDs)
        end
    end

    if self.db.profile.moveElementalTradeGoodsAwakened then
        if self.db.profile.showcoloredCategories then
            Result["|cff644bffElemental Trade Goods - Awakened|r"] = AddToSet(ElementalTradeGoodsAwakenedIDs)
        else
            Result[unescape("|cff644bffElemental Trade Goods - Awakened|r")] = AddToSet(ElementalTradeGoodsAwakenedIDs)
        end
    end

    if self.db.profile.moveTreasureSacks then
        if self.db.profile.showcoloredCategories then
            Result["|cfffff000Treasure Sacks|r"] = AddToSet(TreasureSacksIDs)
        else
            Result[unescape("|cfffff000Treasure Sacks|r")] = AddToSet(TreasureSacksIDs)
        end
    end

    if self.db.profile.moveProfessionGear then
        if self.db.profile.showcoloredCategories then
            Result["|cffffd883Profession Gear|r"] = AddToSet(ProfessionGearIDs)
        else
            Result[unescape("|cffffd883Profession Gear|r")] = AddToSet(ProfessionGearIDs)
        end
    end

    if self.db.profile.moveReputationItems then
        if self.db.profile.showcoloredCategories then
            Result["|cff1eff00Reputation Items|r"] = AddToSet(ReputationItemsIDs)
        else
            Result[unescape("|cff1eff00Reputation Items|r")] = AddToSet(ReputationItemsIDs)
        end
    end

    if self.db.profile.movePreEventGear then
        if self.db.profile.showcoloredCategories then
            Result["|cff0050a2PreEvent Gear|r"] = AddToSet(PreEventGearIDs)
        else
            Result[unescape("|cff0050a2PreEvent Gear|r")] = AddToSet(PreEventGearIDs)
        end
    end

    if self.db.profile.moveFortuneCards then
        if self.db.profile.showcoloredCategories then
            Result["|cff6cff00Fortune Cards|r"] = AddToSet(FortuneCardsIDs)
        else
            Result[unescape("|cff6cff00Fortune Cards|r")] = AddToSet(FortuneCardsIDs)
        end
    end

    if self.db.profile.moveAlchemy then
        if self.db.profile.showcoloredCategories then
            Result["|cff91ffe9Alchemy|r"] = AddToSet(AlchemyIDs)
        else
            Result[unescape("|cff91ffe9Alchemy|r")] = AddToSet(AlchemyIDs)
        end
    end

    if self.db.profile.moveEnchanting then
        if self.db.profile.showcoloredCategories then
            Result["|cff96ff00Enchanting|r"] = AddToSet(EnchantingIDs)
        else
            Result[unescape("|cff96ff00Enchanting|r")] = AddToSet(EnchantingIDs)
        end
    end

    if self.db.profile.moveEngineering then
        if self.db.profile.showcoloredCategories then
            Result["|cff787878Engineering|r"] = AddToSet(EngineeringIDs)
        else
            Result[unescape("|cff787878Engineering|r")] = AddToSet(EngineeringIDs)
        end
    end

    if self.db.profile.moveCraftingPotions then
        if self.db.profile.showcoloredCategories then
            Result["|cffebb134Crafting Potions|r"] = AddToSet(CraftingPotionsIDs)
        else
            Result[unescape("|cffebb134Crafting Potions|r")] = AddToSet(CraftingPotionsIDs)
        end
    end
...
setFilter.uiName = "|cffA00000Dragonflight|r"
setFilter.uiDesc = "Items from the Dragonflight\n|cffFFD800Filter version: 1.1.15|r"
...
function setFilter:OnInitialize()
    self.db = AdiBags.db:RegisterNamespace("Dragonflight", {
        profile = {
            ...
            moveHerbs = false,
            moveElementalTradeGoodsRousing = false,
            moveElementalTradeGoods = true,
            moveLeather = false,
            moveJewelcrafting = false,
            moveCloth = false,
            moveMining = false,
            moveProfessionKnowledge = true,
            moveElementalTradeGoodsAwakened = false,
...
function setFilter:GetOptions()
    return {
        moveFood = {
            name = "Food",
            desc = "Food added in the Dragonflight expansion",
            type = "toggle",
            order = 10
        },
        ...
        moveInscription = {
            name = "Inscription",
            desc = "Crafting Reagents of the 'Inscription' category",
            type = "toggle",
            order = 20
        },
        ...
        movePotions = {
            name = "Potions",
            desc = "Potions added in the Dragonflight expansion",
            type = "toggle",
            order = 30
        },
        ...
        moveCooking = {
            name = "Cooking",
            desc = "Crafting Reagents of the 'Cooking' category",
            type = "toggle",
            order = 40
        },

        moveGeneralCraftingReagents = {
            name = "General Crafting Reagents",
            desc = "General Crafting Reagents, used by multiple professions",
            type = "toggle",
            order = 50
        },

        moveDrakewatcherManuscripts = {
            name = "Drakewatcher Manuscripts",
            desc = "Drakewatcher Manuscripts for learning new customizations for your Dragonriding mounts",
            type = "toggle",
            order = 60
        },
        ...
        movePreEventCurrency = {
            name = "PreEvent Currency",
            desc = "Currency-like items dropped in the Dragonflight Pre-Patch Event",
            type = "toggle",
            order = 80
        },
        ...
        moveHerbs = {
            name = "Herbs",
            desc = "Crafting Reagents of the 'Herbs' category",
            type = "toggle",
            order = 90
        },

        moveElementalTradeGoodsRousing = {
            name = "Elemental Trade Goods - Rousing",
            desc = "Rousing Elemental Trade Goods (use this OR the combined category)",
            type = "toggle",
            order = 100
        },

        moveElementalTradeGoods = {
            name = "Elemental Trade Goods",
            desc = "Awakened and Rousing Elemental Trade Goods",
            type = "toggle",
            order = 110
        },
        ...
        moveLeather = {
            name = "Leather",
            desc = "Crafting Reagents of the 'Leather' category",
            type = "toggle",
            order = 120
        },
        ...
        moveJewelcrafting = {
            name = "Jewelcrafting",
            desc = "Crafting Reagents of the 'Jewelcrafting' category",
            type = "toggle",
            order = 130
        },
        ...
        moveCloth = {
            name = "Cloth",
            desc = "Crafting Reagents of the 'Cloth' category",
            type = "toggle",
            order = 140
        },
        ...
        moveMining = {
            name = "Mining",
            desc = "Crafting Reagents of the 'Mining' category",
            type = "toggle",
            order = 150
        },
        ...
        moveProfessionKnowledge = {
            ...
            desc = "Items that provide profession knowledge",
            type = "toggle",
            order = 160
        },

        moveElementalTradeGoodsAwakened = {
            name = "Elemental Trade Goods - Awakened",
            desc = "Awakened Elemental Trade Goods (use this OR the combined category)",
            type = "toggle",
            order = 170
        },
        ...
        moveProfessionGear = {
            ...
            desc = "Specialized gear, which improves your profession",
            type = "toggle",
            order = 190
        },
        ...
        moveReputationItems = {
            ...
            desc = "Contains Items which can be directly traded in for reputation/renown, as well as items needed for Wrathion & Sabellian",
            type = "toggle",
            order = 200
        },
        ...
        movePreEventGear = {
            name = "PreEvent Gear",
            desc = "Gear dropped or bought in the Dragonflight Pre-Patch Event",
            type = "toggle",
            order = 210
        },
        ...
        moveAlchemy = {
            name = "Alchemy",
            desc = "Crafting Reagents of the 'Alchemy' category (flasks only, you may also want to enable Herbs)",
            type = "toggle",
            order = 230
        },
        ...
        moveEnchanting = {
            name = "Enchanting",
            desc = "Crafting Reagents of the 'Enchanting' category",
            type = "toggle",
            order = 240
        },
        ...
        moveEngineering = {
            name = "Engineering",
            desc = "Crafting Reagents of the 'Engineering' category",
            type = "toggle",
            order = 250
        },
        ...
        moveCraftingPotions = {
            ...
            desc = "Potions which improve crafting",
            type = "toggle",
            order = 260
        },
